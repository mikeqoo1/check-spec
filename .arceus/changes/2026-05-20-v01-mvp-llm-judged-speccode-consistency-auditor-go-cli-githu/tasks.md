...
- [x] 在本 repo dogfood：對本 change（commit `36071d0`）跑 verify，產出 `docs/audits/v0.1-self-audit.md` — Opus 4.7 判定 49/51 task done、10/11 criteria PASS，正確抓到 dogfood + status flip 兩個自我回報為 pending 的項目，並指出舊 audit doc 是 zero-state（即本 commit 修正的對象）

## Phase 9 — Acceptance gate

- [x] 走查 spec.md「驗收條件」每一項，逐條打勾並貼證據（檔案路徑 / 測試名稱）
- [x] 將本 change `status` 從 `active` 轉為 `completed`：使用者確認 dogfood 後執行 `arceus change status <id> completed`
